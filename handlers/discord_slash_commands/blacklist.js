...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('blacklist')
        .setDescription('Blacklist A Discord User From Using This Gameplay Bot. [Admin User Command]')
        ...
        if (interactionDetails.member.roles.cache.some((discordUserRole) => interactionWhistelistedRolesID.includes(discordUserRole.id)) == true) {
            ...
            if (discordUserIDRegex.test(discordUserID) == true) {
                ...
                if (discordUser != undefined) {
                    ...
                    if (discordUser.roles.cache.has(gameplayBotBlacklistedRoleID) == false) {
                        if (discordBot.guilds.cache.get(guildID).members.me.permissions.has('ManageRoles', true) == true) {
                            await discordUser.roles.add(gameplayBotBlacklistedRoleID).then(async () => {
                                await interactionDetails.editReply({ content: '```Blacklisted ' + discordUser.displayName + ' from using this gameplay bot.```' }).then(() => {
                        ...
                        await interactionDetails.editReply({ content: '```This user is already blacklisted from using this gameplay bot!```' }).then(() => {

                            interactionResultDetails.result = false, interactionResultDetails.failedReason = 'User is already blacklisted from using this gameplay bot!';
